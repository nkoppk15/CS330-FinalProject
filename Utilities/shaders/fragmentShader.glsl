...
void main()
{
   if(bUseLighting == true)
   {
      ...
      vec3 phongResult = CalcDirectionalLight(sunLight, lightNormal, viewDirection);
...
vec3 CalcLightSource(LightSource light, vec3 lightNormal, vec3 vertexPosition, vec3 viewDirection)
{
   ...
   float specularComponent = pow(max(dot(viewDirection, reflectDir), 0.0), material.shininess * light.focalStrength);
   specular = light.specularColor * light.specularIntensity * specularComponent * material.specularColor;
  
   return ambient + diffuse + specular;
}

// calculates the color and direction of sunlight
vec3 CalcDirectionalLight(DirectionalLight light, vec3 normal, vec3 viewDirection)
{
    ...
    vec3 specular = spec * light.specularColor * material.specularColor;
