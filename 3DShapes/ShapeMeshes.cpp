...
//	LoadRoundedBoxMesh()
//
//	Create a rectangular box mesh with flat 45-degree
//  beveled (chamfered) edges and corners by specifying
//  the vertices, and store it in a VAO/VBO.  The normals
//  and texture coordinates are also set.  The mesh is
//  built as a unit cube (+/- 0.5 on each axis) so it can
//  be scaled like the other shape meshes when drawn.
//
//	Correct triangle drawing command:
//
//	glDrawArrays(GL_TRIANGLES, 0, m_RoundedBoxMesh.nVertices);
//
//	Citation:
//	Vertex/normal generation logic for the beveled edges
//	and corners was developed with the assistance of an
//	AI language model (OpenAI / Anthropic Claude), [5/31/2026],
//	following the existing mesh-loading conventions used
//	in this ShapeMeshes class.
///////////////////////////////////////////////////

void ShapeMeshes::LoadRoundedBoxMesh()
{
	// Make sure <vector> and <cmath> are included at the top of the file.

	// Half-extent of the unit box (matches your prism's ±0.5 convention)
	const float h = 0.5f;
	// Bevel size: how far the chamfer cuts in from each edge/corner.
	// Smaller = sharper edges; larger = more rounded look. Must be < h.
	const float b = 0.15f;

	const float in = h - b; // inner coordinate where the bevel begins

	std::vector<GLfloat> verts;

	// Helper: push one vertex (pos.xyz, normal.xyz, uv.xy)
	auto V = [&](float px, float py, float pz,
		float nx, float ny, float nz,
		float u, float vt)
		{
			verts.insert(verts.end(),
				{ px, py, pz, nx, ny, nz, u, vt });
		};

	// Helper: push a quad as two triangles (CCW) with a single face normal
	auto Quad = [&](
		...
		float nx, float ny, float nz)
		{
			V(ax, ay, az, nx, ny, nz, 0.0f, 0.0f);
			V(bx, by, bz, nx, ny, nz, 1.0f, 0.0f);
			V(cx, cy, cz, nx, ny, nz, 1.0f, 1.0f);

			V(ax, ay, az, nx, ny, nz, 0.0f, 0.0f);
			V(cx, cy, cz, nx, ny, nz, 1.0f, 1.0f);
			V(dx, dy, dz, nx, ny, nz, 0.0f, 1.0f);
		};

	// Helper: push a triangle (used for the corner cuts)
	auto Tri = [&](
		float ax, float ay, float az,
		float bx, float by, float bz,
		float cx, float cy, float cz,
		float nx, float ny, float nz)
		{
			V(ax, ay, az, nx, ny, nz, 0.0f, 0.0f);
			V(bx, by, bz, nx, ny, nz, 1.0f, 0.0f);
			V(cx, cy, cz, nx, ny, nz, 0.5f, 1.0f);
		};

	const float s = 0.70710678f; // sqrt(2)/2, for 45-degree bevel normals

	// ---- 6 main faces (each shrunk inward by b on its in-plane axes) ----

	// +Z face (front)
	Quad(-in, -in, h, in, -in, h, in, in, h, -in, in, h, 0, 0, 1);
	// -Z face (back)
	Quad(in, -in, -h, -in, -in, -h, -in, in, -h, in, in, -h, 0, 0, -1);
	// +X face (right)
	Quad(h, -in, in, h, -in, -in, h, in, -in, h, in, in, 1, 0, 0);
	// -X face (left)
	Quad(-h, -in, -in, -h, -in, in, -h, in, in, -h, in, -in, -1, 0, 0);
	// +Y face (top)
	Quad(-in, h, -in, in, h, -in, in, h, in, -in, h, in, 0, 1, 0);
	// -Y face (bottom)
	Quad(-in, -h, in, in, -h, in, in, -h, -in, -in, -h, -in, 0, -1, 0);

	// ---- 12 edge bevels (flat 45-degree chamfers) ----

	// 4 edges parallel to X axis (vary in Y and Z signs)
	Quad(-in, -h, in, in, -h, in, in, -in, h, -in, -in, h, 0, -s, s); // bottom-front
	Quad(-in, -in, -h, in, -in, -h, in, -h, -in, -in, -h, -in, 0, -s, -s); // bottom-back
	Quad(-in, in, h, in, in, h, in, h, in, -in, h, in, 0, s, s); // top-front
	Quad(-in, h, -in, in, h, -in, in, in, -h, -in, in, -h, 0, s, -s); // top-back

	// 4 edges parallel to Y axis (vary in X and Z signs)
	Quad(in, -in, h, in, in, h, h, in, in, h, -in, in, s, 0, s); // right-front
	Quad(h, -in, -in, h, in, -in, in, in, -h, in, -in, -h, s, 0, -s); // right-back
	Quad(-h, -in, in, -h, in, in, -in, in, h, -in, -in, h, -s, 0, s); // left-front
	Quad(-in, -in, -h, -in, in, -h, -h, in, -in, -h, -in, -in, -s, 0, -s); // left-back

	// 4 edges parallel to Z axis (vary in X and Y signs)
	Quad(in, -h, -in, in, -h, in, h, -in, in, h, -in, -in, s, -s, 0); // right-bottom
	Quad(in, h, in, in, h, -in, h, in, -in, h, in, in, s, s, 0); // right-top
	Quad(-h, -in, -in, -h, -in, in, -in, -h, in, -in, -h, -in, -s, -s, 0); // left-bottom
	Quad(-h, in, in, -h, in, -in, -in, h, -in, -in, h, in, -s, s, 0); // left-top

	// ---- 8 corner cuts (triangles connecting three edge bevels) ----
	const float c = 0.57735027f; // 1/sqrt(3), for corner normals

	// +X +Y +Z
	Tri(in, in, h, h, in, in, in, h, in, c, c, c);
	// +X +Y -Z
	Tri(in, in, -h, in, h, -in, h, in, -in, c, c, -c);
	// +X -Y +Z
	Tri(in, -in, h, in, -h, in, h, -in, in, c, -c, c);
	// +X -Y -Z
	Tri(in, -in, -h, h, -in, -in, in, -h, -in, c, -c, -c);
	...
	// ---- Upload to GPU (same pattern as your other meshes) ----
	m_RoundedBoxMesh.nVertices =
		static_cast<GLuint>(verts.size() /
			(g_FloatsPerVertex + g_FloatsPerNormal + g_FloatsPerUV));

	glGenVertexArrays(1, &m_RoundedBoxMesh.vao);
	glBindVertexArray(m_RoundedBoxMesh.vao);

	glGenBuffers(1, m_RoundedBoxMesh.vbos);
	glBindBuffer(GL_ARRAY_BUFFER, m_RoundedBoxMesh.vbos[0]);
...
void ShapeMeshes::LoadPrismMesh()
{
	...
	GLfloat verts[] = {
		...
		//Top Face				//Positive Y Normal		
		1.0f, 0.0f, 0.5f,     0.707f, 0.707f, 0.0f,
		0.0f, 1.0f, 0.5f,     0.707f, 0.707f, 0.0f,
		0.0f, 1.0f, -0.5f,    0.707f, 0.707f, 0.0f,
		1.0f, 0.0f, -0.5f,    0.707f, 0.707f, 0.0f
	};
	...
	GLuint indices[] = {
		...
		// Side 1 (Bottom Rectangle)
		6, 7, 8,
		8, 9, 6,

		// Side 2 (Left Rectangle)
		10, 11, 12,
		12, 13, 10,

		// Side 3 (Hypotenuse Rectangle)
		14, 15, 16,
		16, 17, 14
	};
	...
	glGenBuffers(1, &m_PrismMesh.vbos[0]);
	glGenBuffers(1, &m_PrismMesh.ebo);
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_PrismMesh.ebo);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);
...
//	DrawRoundedBoxMesh()
//
//	Transform and draw the rounded box mesh to the window.
//
///////////////////////////////////////////////////
void ShapeMeshes::DrawRoundedBoxMesh()
{
	glBindVertexArray(m_RoundedBoxMesh.vao);
